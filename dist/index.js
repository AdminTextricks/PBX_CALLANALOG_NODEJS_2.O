...
const options = {
  ...
  key: fs.readFileSync("/etc/letsencrypt/live/socket.callanalog.com-0004/privkey.pem")
};
  ...
  ws.on("message", message => {
    ...
    if (data.action === "fetchUserActive") {
      ...
      const fetchUserActive = callback => {
        connection.query("SELECT status FROM users WHERE id = ?", [id], (err, results) => {
          if (err) {
    ...
    if (data.action === "fetchDocStatus") {
      ...
      const fetchDocStatus = callback => {
        connection.query(`SELECT CASE WHEN is_verified_doc = 0 THEN 0 WHEN is_verified_doc = 1 THEN 1 WHEN is_verified_doc = 2 THEN 2 WHEN is_verified_doc = 3 THEN 3 ELSE 'Unknown status' END AS status_message
          FROM users
          WHERE company_id = ?`, [userId], (err, results) => {
          if (err) {
            return callback(null);
          }
...
`;
      const GET_NOTIFICATION = ({
        ...
        if (["super-admin", "noc", "support"].includes(user_type)) {
          connection.query(superadmin_query, [user_type, user_type], (err, results) => {
            if (err) {
              return callback(null);
            }
      ...
      setInterval(() => {
        GET_NOTIFICATION({
          ...
          if (Array.isArray(newNotification) && newNotification.length > 0) {
            const unreadCount = newNotification[0]?.notification_recipients_count;
            const data = JSON.parse(newNotification[0].data);
            if (unreadCount !== prevUnreadCount || JSON.stringify(data) !== JSON.stringify(prevData)) {
